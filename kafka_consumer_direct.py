#!/usr/bin/env python3
"""
Secure Kafka consumer for Pathik-streamed content.
This script reads messages from a Kafka topic where Pathik has streamed crawled content.
It includes all security fixes and enhancements.
...
def main():
    ...
    parser.add_argument("--max-bytes", type=int, 
                        default=get_env_int("KAFKA_MAX_BYTES", 20 * 1024 * 1024),
                        help="Maximum fetch size (default: 20MB)")
    parser.add_argument("--max-partition-bytes", type=int, 
                        default=get_env_int("KAFKA_MAX_PARTITION_BYTES", 10 * 1024 * 1024),
                        help="Maximum bytes per partition (default: 10MB)")
    parser.add_argument("--receive-buffer", type=int, 
                        default=get_env_int("KAFKA_RECEIVE_BUFFER", 128 * 1024),
                        help="Socket receive buffer size (default: 128KB)")
    ...
    if args.max_partition_bytes <= 0:
        print("Error: max-partition-bytes must be positive", file=sys.stderr)
        sys.exit(1)
    
    if args.receive_buffer <= 0:
        print("Error: receive-buffer must be positive", file=sys.stderr)
        sys.exit(1)
    ...
    config: Dict[str, Any] = {
        ...
        'max_partition_fetch_bytes': args.max_partition_bytes,
        'receive_buffer_bytes': args.receive_buffer,
    }
    ...
    if args.session:
        print(f"Filtering for session ID: {args.session}")
    
    # Print buffer configurations
    print("\nBuffer configurations:")
    print(f"Max fetch bytes: {args.max_bytes:,} bytes")
    print(f"Max partition fetch bytes: {args.max_partition_bytes:,} bytes")
    print(f"Receive buffer size: {args.receive_buffer:,} bytes")
